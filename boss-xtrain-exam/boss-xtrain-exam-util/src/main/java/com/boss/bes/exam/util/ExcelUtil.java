...
import boss.xtrain.core.annotation.excel.Excel.*;
import boss.xtrain.core.annotation.excel.Excels;
import com.boss.bes.exam.text.Convert;
...
public class ExcelUtil<T> {
    ...
    public List<T> importExcel(String sheetName, InputStream is) throws Exception {
        ...
        if (StringUtil.isNotEmpty(sheetName)) {
            ...
            sheetExcel = wb.getSheet(sheetName);
        } else {
            // 如果传入的sheet名不存在则默认指向第1个sheet.
            sheetExcel = wb.getSheetAt(0);
        }
        ...
        int rows = sheetExcel.getPhysicalNumberOfRows();

        if (rows > 0) {
            // 定义一个map用于存放excel列的序号和field.
            Map<String, Integer> cellMap = new HashMap<>();
            // 获取表头
            Row heard = sheetExcel.getRow(0);
